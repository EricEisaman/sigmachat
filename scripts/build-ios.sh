...
[ -n "${FLUFFYCHAT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${FLUFFYCHAT_ORIG_GROUP}.app/group.${FLUFFYCHAT_NEW_GROUP}.app/g" "ios/SigmaChat Share/SigmaChat Share.entitlements"
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for sigmachat.app and sigmachat.app.SigmaChat-Share)
...
[ -n "${FLUFFYCHAT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/sigmachat.ipa"
  rm -rf "${TMPDIR}"
}
