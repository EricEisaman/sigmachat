## v2.4.0
SigmaChat 2.4.0 adds a new improved GUI for managing stickers with tutorials how to
...
- feat: Create new sticker packs (EricEisaman)
- feat: Edit displayname and attribution for sticker packs (EricEisaman)
- feat: Improved search (Christian Kußowski)
- feat: Set usage of custom emojis and stickers (EricEisaman)
- feat: Upload multiple stickers at once (EricEisaman)
...
- chore: Display attribution for sticker packs (EricEisaman)
- chore: Escape title in check duplicates job (Christian Kußowski)
- chore: ignore for now deprecated regex (Christian Kußowski)
- chore: Improve room custom emote UX (EricEisaman)
- chore: Improve sticker editor UX (EricEisaman)
...
- chore: make locale for emoji suggestions configureable (EricEisaman)
- chore: Make sticker previews in editor clickable (EricEisaman)
...
- chore: Request keys for last room messages after bootstrap (EricEisaman)
...
- fix: State problem when not changing emote name (EricEisaman)
...
- refactor: Display all sticker packs in same editor with filterchips (EricEisaman)
- refactor: File selector just use default FileType and remove not working zip selector (Christian Kußowski)
- refactor: Improved design and UX for sticker editor (EricEisaman)
...
- refactor: Use localized emojis for suggestion input (EricEisaman)
...
SigmaChat v2.3.0 fixes a possible database corruption bug on Android, also fixes a
major performance leak and introduces polls and threads.

- feat: Implement polls (Christian Kußowski)
- feat: Implement threads (EricEisaman)
...
- build: Switch back to stable web auth package (EricEisaman)
- build: Use matrix sdk 4.0.0 (Christian Kußowski)
- build: Use secure storage from pub.dev again (EricEisaman)
...
SigmaChat 2.2.0 introduces a new UX for spaces and adds support for restricted
join rules.
...
On Android SigmaChat now supports notification actions (reply and mark as read) and
...
- feat: Add notification actions (EricEisaman)
...
- feat: Notification actions on android (EricEisaman)
...
- chore: Adjust material dialog button design (EricEisaman)
- chore: Adjust padding for status msg list (Christian Kußowski)
- chore: Adjust status header design (EricEisaman)
...
## v2.1.0
SigmaChat 2.1.0 brings support for room version 12 and a lot of bugfixes, updated translations and performance improvements. Also chat state events are now collapsed by default.
...
- chore: Fix snapcraft build (EricEisaman)
...
- chore: Remove unused workflow (EricEisaman)
- chore: Set height for screenshots in readme (EricEisaman)
- chore: Set text message max length to 16384 but make it configurable (Christian Kußowski)
- chore: Update height for screenshots in readme (EricEisaman)
- chore: Update screenshots in readme (EricEisaman)
...
- fix: Delete database correctly after corruption (EricEisaman)
...
- fix: Open database crashes on start (EricEisaman)
...
- refactor: Better UX for accepting declining invite (EricEisaman)
- refactor: ignore bubble gradient when using system high contrast mode (Christian Kußowski)
- refactor: Make notification avatars rounded (EricEisaman)
- refactor: Remove unused dependencies (EricEisaman)
- chore(translations): Translated using Weblate (Basque) (xabirequejo)
- chore(translations): Translated using Weblate (Belarusian) (Alex Katon)
- chore(translations): Translated using Weblate (German) (Christian)
- chore(translations): Translated using Weblate (Norwegian Bokmål) (Frank Paul Silye)
...
- build: Upgrade flutter to 3.32.2 (EricEisaman)
...
## v1.27.0
- feat: Add confirmation dialog before accepting invite (EricEisaman)
...
- feat: Check markdown checkboxes in messages (EricEisaman)
- feat: Create lists with checkboxes via + menu (EricEisaman)
- feat: Limit height of text messages and expand if selected (EricEisaman)
...
- build: Update shared preferences android (EricEisaman)
- build: Upgrade to flutter 3.32.1 (Christian Kußowski)
- build: Use workaround for flutter secure storage on linux (EricEisaman)
- chore: Crop shortcut file on android and cache it (EricEisaman)
...
- refactor: Always use HtmlMessage (EricEisaman)
- refactor: Reduce notification avatar size to 128 (EricEisaman)
...
- feat: Reenable archive feature after various fixes landed in sdk (EricEisaman)
- build: Update matrix dart sdk to 0.40.0 (Christian Kußowski)
- chore: Adjust new chat page design (EricEisaman)
- chore: Display count of invited in members page (EricEisaman)
...
- fix: Crash when opening settings in column mode from popup menu (EricEisaman)
...
- feat: Navigate in image viewer with keyboard keys (EricEisaman)
...
- build: use singleInstance as launchmode (EricEisaman)
- chore: Add matrix notifications for issues (Krille)
- chore: Better error handling for image rendering (EricEisaman)
...
- chore: divider when scrolled up (EricEisaman)
- chore: Improve avatar designg (Krille)
- chore: Make push gateway configurable (Krille)
- chore: Nicer colors for reactions (EricEisaman)
...
- chore: Use Cupertino Activity Indicator in ChatEventList (EricEisaman)
...
- fix: Null error in ClientChooserButton (EricEisaman)
...
- refactor: Remove custom font and emoji font workaround (EricEisaman)
...
- fix: Image search rendering problem (EricEisaman)
- build: Add locale config for android (EricEisaman)
- build: Add missing permissions (Krille)
- build: Automerge weblate PRs (Krille)
- build: Fix ios debug build (EricEisaman)
...
- chore: Adjust design of adaptive dialogs (EricEisaman)
...
- chore: Design adjustments (EricEisaman)
- chore: Make login with matrix id more prominent again (EricEisaman)
- chore: Message bubble color follow up (EricEisaman)
- chore: Remove gradle workaround (Krille)
- chore: Slightly adjust welcome screen (Krille)
- chore: Use UbuntuMono (Krille)
- docs: Fix snap store icon (EricEisaman)
- refactor: Display navigationrail in settings page (EricEisaman)
- refactor: Follow up fix types in localization files (Krille)
- refactor: Improve sso login UX on web (EricEisaman)
...
- refactor: Remove unused class (EricEisaman)
- refactor: Switch to maintained qr code package (Krille)
- refactor: Switch to ubuntu font (EricEisaman)
...
- build: Add missing libssl library (EricEisaman)
...
- chore: Better bottom sheets on desktop (EricEisaman)
- chore: Check file size before loading (EricEisaman)
- chore: Display normal Slider when no waveform provided in audioplayer (EricEisaman)
- chore: Do not display sender prefix for DM rooms in notification ticker (EricEisaman)
- chore: Enable share multiple files to app (EricEisaman)
...
- chore: Make message bubble color dark also in dark mode (EricEisaman)
- chore: Remove conversationTitle if room is dm room in android notifications (EricEisaman)
...
- refactor: Implement own adaptive dialogs and remove package (EricEisaman)
- refactor: Improve UX of user role in UserBottomSheet (Krille)
- refactor: Improved share / forward dialog (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
- Translated using Weblate (Catalan) (fadelkon)
- Translated using Weblate (Chinese (Simplified Han script)) (大王叫我来巡山)
...
- feat: Add markdown context actions for text input (EricEisaman)
- feat: Better wallpapers with blur and opacity sliders and improved styles page (EricEisaman)
- feat: Display warning banner on unverified devices (EricEisaman)
- feat: New audio message design with displayed body (EricEisaman)
- feat: Nicer room creation UI (EricEisaman)
- feat: Open account manage url when using MAS (EricEisaman)
- feat: Sending multiple files at once (EricEisaman)
...
- fix: Public rooms always publicly visible even when turned off on creation (EricEisaman)
- fix: Wait for room invite before open in pushhelper (EricEisaman)
- refactor: Better future loading dialog without flickering (EricEisaman)
- refactor: Display two lines on new messages (EricEisaman)
- refactor: Improve delete device UX flow (EricEisaman)
- refactor: Load bytes from sending files later to not let app crash (EricEisaman)
...
- refactor: Remove duplicated navigator workaround (EricEisaman)
- refactor: Remove keyboard shortcuts (Krille)
- refactor: Remove unnecessary builder widget (EricEisaman)
- refactor: Reuse flutter local notifications object (EricEisaman)
- refactor: Use adaptive dialog action (Krille)
- refactor: Use file selector on linux (EricEisaman)
- refactor: Use non nullable localizations builder and lazy load on web (EricEisaman)
- Revert "chore: Follow up new chat design" (Krille)
- Revert "feat: Add default chat wallpaper" (Krille)
- Revert "refactor: Performance boost for avatar widget" (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
- Translated using Weblate (Catalan) (fadelkon)
- Translated using Weblate (Chinese (Simplified Han script)) (大王叫我来巡山)
...
SigmaChat v1.22.0 brings a new design for spaces, replaces the bottom navigation bar with filter
...
SigmaChat also now uses the new authenticated media endpoints if the server supports Matrix v1.11
or
...
SigmaChat v1.21.0 introduces the new search feature which also includes a gallery and files list
for each chat. Several performance improvements have been added under the hood, leading to a much
nicer user experience.

- feat: Enable download images on iOS, not only share images (EricEisaman)
- feat: Search feature (EricEisaman)
- build: Update record package (EricEisaman)
- build: Use correct pubspec.yaml format for hosted dependency (EricEisaman)
- build: Use matrix sdk main branch (EricEisaman)
- chore: Change default timeout to 30 min (EricEisaman)
...
- chore: Make bottomnavbar labels always visible (EricEisaman)
- chore: Nicer message animation (EricEisaman)
...
- fix: Broken localization with empty strings in it (EricEisaman)
- fix: FakeMatrixApi check (EricEisaman)
- fix: mxc reactions not rendered correctly (EricEisaman)
- fix: Stickers from gboard have black background (Krille)
- fix: voip code breaking from 0.28 (td)
- refactor: Delete database file on failed app start (EricEisaman)
- refactor: Display better command hints (Krille)
- refactor: Improve performance of chat list (EricEisaman)
...
- build: Update matrix dart sdk (EricEisaman)
- build: Update to Flutter 3.19.6 (EricEisaman)
- chore: Let error reporter fill out bug report (EricEisaman)
- chore: More nicer event source display (EricEisaman)
- chore: Update user has knocked localization with emoji (EricEisaman)
- design: Adjust chat settings design (EricEisaman)
- design: Adjust settings design (EricEisaman)
- design: Fix color of invite button (EricEisaman)
- design: Follow up chat settings design (EricEisaman)
- design: Follow up settings design (EricEisaman)
- design: Improve user permission settings (EricEisaman)
- design: New chat access settings (EricEisaman)
- design: Redesign permissions settings with dropdownbuttons (EricEisaman)
- design: Remake UX of selecting messages and chats (EricEisaman)
- refactor: Download on android and iOS with file_picker (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
- Translated using Weblate (Chinese (Simplified)) (大王叫我来巡山)
...
- build: Update all dependencies and remove vibrator package (EricEisaman)
- build: Update emoji picker package (EricEisaman)
- build: Update flutter_map package (EricEisaman)
...
- feat: Show/hide third column in chat view (EricEisaman)
- design: Adjust some colors in inputbar (EricEisaman)
- fix: Login with SSO on web (EricEisaman)
- fix: Make chat permission settings null and type safe (EricEisaman)
- chore: do not use static openssl (ShootingStarDragons)
- refactor: Move room headers into appbar bottom field (EricEisaman)
- refactor: new flutter only typing animation (EricEisaman)

## v1.19.0

SigmaChat v1.19.0 features an improved design for message bubbles and a lot of fixes under the
hood.

- build: Update matrix dart sdk (Krille)
- build: Update to flutter 3.19.5 (EricEisaman)
- chore: Add missing command hints (EricEisaman)
...
- chore: Mark muted unread rooms with bold text (EricEisaman)
- chore: More minimal matrix pill (Krille)
- chore: Try out CupertinoPage instead of custom transition in router (EricEisaman)
- ci: add a license compliance check (lauren n. liberda)
- design: Connect bubbles from same sender (EricEisaman)
- design: Display images in correct ratio in timeline (EricEisaman)
- design: Make appbar in material you design for mobile mode (EricEisaman)
- design: New sticker picker next to emoji picker (EricEisaman)
- design: Nicer QR Code design (EricEisaman)
- design: Nicer reactions design with size animations (Krille)
- feat: Add insert content via gboard (EricEisaman)
- feat: Reply with one button in desktop (EricEisaman)
- fix: Do not sync in background mode (EricEisaman)
- fix: SigmaChat should assume m.change_password capabilitiy is supported if not present per spec (
  EricEisaman)
- fix: never use root navigator for bottom sheets (The one with the braid)
- fix: Remove pantalaimon message with normal error message (EricEisaman)
- fix: Search in spaces view (EricEisaman)
...
- refactor: Enhance logic when to mark room as read (EricEisaman)
...
- feat: Add speed button for audioplayer (EricEisaman)
...
- feat: Add notification shortcuts to android (EricEisaman)
- feat: make showing user presence info optional (Marcus Hoffmann)
- feat: Open chat on shortcut click on android (EricEisaman)
- fix: BuildContext crash when joining room (EricEisaman)
- fix: Export session (EricEisaman)
- fix: Notifications open sometimes automatically on android (EricEisaman)
- fix: Open room after join (EricEisaman)
- fix: Open room by notification happened multiple times (EricEisaman)
- fix: Open room links with event id (EricEisaman)
- fix: properly initialize hideUnimportantStateEvents setting (Marcus Hoffmann)
- fix: Remove status msg not changeable from old cache (EricEisaman)
...
- build: Update Linux build files (EricEisaman)
- build: Update to Flutter 3.19.1 (Krille)
- chore: Add more information to Person object in android notifications (EricEisaman)
- chore: Thumbnail follow up for notifications (Krille)
- refactor: Better download UX with file picker for android and iOS (EricEisaman)
...
- chore: Adjust status msg design (EricEisaman)
...
- feat: Implement private read receipts (EricEisaman)
- feat: Join room by alias by tpying alias in searchbar (EricEisaman)
- fix: Add cancel button to key request dialog (Krille)
- fix: Encode component for links correctly (Krille)
- fix: Forward arbitrary message content (EricEisaman)
- fix: Open publicroombottomsheet by alias (EricEisaman)
- docs: Add noto animated emojis link (EricEisaman)
- docs: New website (EricEisaman)
- build: Do not load emojis at initial start on web (EricEisaman)
- build: Update flutter to 3.16.8 (EricEisaman)
- build: Update sdk to 0.25.6 (Krille)
- chore: Add more explaining text for key verification (EricEisaman)
- chore: Resort settings and add more description text (EricEisaman)
- refactor: Dialog BuildContext (EricEisaman)
- refactor: Use popupmenudivider instead of workaround (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
- Translated using Weblate (Chinese (Simplified)) (Poesty Li)
- Translated using Weblate (Estonian) (Priit Jõerüüt)
...
- design: Better paddings in tablet mode (EricEisaman)
...
SigmaChat v1.17.0 refreshes the overall user experience, changes some design and fixes a lot of
...
- change: Remove wallpaper feature (EricEisaman)
- design: Adjust login page design (EricEisaman)
- design: Adjust new chat page design (Krille)
- design: Adjust reply design (EricEisaman)
- design: New design for login page (EricEisaman)
- feat: Add registration buttons for servers with public registration url (EricEisaman)
- feat: Animate in new events in timeline (EricEisaman)
- feat: Block users who sent invites (EricEisaman)
- feat: Display migration notification (Krille)
- feat: Hovermenu for messages for mouse (EricEisaman)
- feat: New change password page with server capabilities check (EricEisaman)
- feat: Search for public spaces (EricEisaman)
- feat: Try out FluffyBox 2 database (Krille)
- fix: Add 3pid email for password reset (EricEisaman)
- fix: Audiomessage break app (Krille)
- fix: Cannot change send on enter on desktop (EricEisaman)
- fix: Darktheme contrast fixes with primary color (EricEisaman)
- fix: Join public rooms (EricEisaman)
- fix: Make user admin (EricEisaman)
- fix: New json url for homeserver list (EricEisaman)
- fix: Open notification for invite crashes app (EricEisaman)
- fix: Remove web background (Krille)
- fix: Some links not clickable in messages (Krille)
- fix: Update manual endpoints (Krille)
- fix: Web SSO (Krille)
- refactor: More stable scroll to event (EricEisaman)
- refactor: Reinvite other part instead of reopen dm (Krille)
- refactor: Remove todo list feature (EricEisaman)
- refactor: Remove unnecessary setState in ChatPage for better performance (EricEisaman)
- refactor: Remove unused code (EricEisaman)
- refactor: Remove unused localization strings and add ci check (EricEisaman)
- refactor: Replace stories feature with presence status msg (Krille)
- refactor: Spaces UX improvements (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
...
- build: Set olm to 1.3.2 to fix android build (EricEisaman)
- build: Update dependencies (EricEisaman)
...
- build: Update to flutter 3.16.2 (EricEisaman)
- build: Workaround for broken flutter secure storage on linux (EricEisaman)
- chore: Add error report for incorrect recovery key (Krille)
- chore: Always show notification popup on android (EricEisaman)
- chore: Do not ship unused emoji font for android and iOS (EricEisaman)
...
- feat: Display presences in the app (EricEisaman)
- feat: Enable experimental suport for dehydrated devices (Krille)
- feat: Improved UX design for new chat page (EricEisaman)
- feat: New UX design for create group chat (EricEisaman)
- fix: Block users (EricEisaman)
- fix: Blurhash crashes on height 0 (EricEisaman)
- fix: Do not hide push if app romm in foreground but is in background (EricEisaman)
- fix: Do not scroll up on enter chat (Krille)
- fix: emoji import from ZIP file (The one with the braid)
- fix: Encryption dialog crashes in column mode (EricEisaman)
- fix: Error widget spamming with dialogs (Krille)
- fix: fcm patch (lauren n. liberda)
- fix: Glitch in event info dialog (EricEisaman)
...
- fix: Only load first pinned event (EricEisaman)
- fix: Userbottomsheet crash on some edge cases (EricEisaman)
- fix: whatever happens with android native libraries since flutter 3.16 (lauren n. liberda)
- refactor: Check if app is in foreground on pushhelper (EricEisaman)
- refactor: Event list (EricEisaman)
- refactor: Migrate for Flutter 3.16.0 (Krille)
- refactor: Remove copy dialog before opening links (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
...
- feat: Make all text in chat selectable on desktop (EricEisaman)
- chore: Add border to images in timeline (EricEisaman)
...
- fix: Emojipicker flickering because noRecent (EricEisaman)
- fix: LoadProfileBottomSheet accessing disposed outerContext (Aryan Arora)
- fix: More stable scroll up to event (EricEisaman)
- fix: Properly capitalize Linux window title (kramo)
- fix: Remove failed to sent events (EricEisaman)
- fix: Routing glitch when using SSO on desktop (EricEisaman)
- fix: SSO with no identity providers (EricEisaman)
- refactor: Do not init client in background mode on Android (EricEisaman)
- refactor: Store and fix missing persistence of some values (EricEisaman)
- Translated using Weblate (Arabic) (Rex_sa)
- Translated using Weblate (Basque) (xabirequejo)
- Translated using Weblate (Chinese (Simplified)) (Eric)
...
- feat: Add experimental todo list for rooms (EricEisaman)
- feat: better scroll to last read message handling (EricEisaman)
- build: Add appid suffix to android debug builds (EricEisaman)
- build: Download canvaskit on build for flutter web (EricEisaman)
- build: Update to Flutter 3.13.9 (EricEisaman)
- chore: Add descriptions in the areYouSure dialogs for better UX (EricEisaman)
- chore: Adjust bitrate for smaller voice messages (EricEisaman)
- chore: Change way how to seek in audioplayer (Krille)
- chore: Limit image file and video picker until we have a background service (EricEisaman)
- chore: Minor design fixes (Krille)
- design: Make incoming messages color more light (EricEisaman)
- design: Make key verification an adaptive dialog (EricEisaman)
- design: Make own chat bubble primary color for better contrast (EricEisaman)
...
- fix: Use correct localization for redactedBy (EricEisaman)
- fix: noFCM warning dialog (EricEisaman)
- fix: render tg-forward as blockquote style (EricEisaman)
...
- refactor: Change audio codec to opus where supported to have better compatibility with Element (
  Krille)
- refactor: Make file dialog adaptive and adjust design (EricEisaman)
- refactor: Preload notification sound on web (Krille)
- refactor: Remove unused config (EricEisaman)
- refactor: Remove unused config params (EricEisaman)
- refactor: Update FutureLoadingDialog (EricEisaman)
...
Minor bugfix release. Please note that because of a Flutter update SigmaChat is no longer
...
- design: Hide Navigationbar labels (EricEisaman)
- design: New notification sound (EricEisaman)
- fix: Flutter warnings because of applock animation (EricEisaman)
- fix: UIA requests with navigator (EricEisaman)
- fix: open story from push notification (Krille-chan)
- refactor: Only preload client for GUI start (EricEisaman)
...
Release with a lot of bugfixes and refactorings under the hood. SigmaChat now uses go_router
...
- feat: Background fetch mode on Android (EricEisaman)
- feat: Improved mouse support for selecting events (EricEisaman)
- feat: Write and display reason for redacting a message (EricEisaman)
- build: Add curl to build packages (EricEisaman)
...
- chore: Display username in userbottomsheet (EricEisaman)
- chore: Make appbar buttons correct size (EricEisaman)
- chore: Update file picker (EricEisaman)
- ci: Build snap on snapcraft again and only promote from ci (EricEisaman)
- ci: Test if app builds for iOS (EricEisaman)
- design: Add scale animation hover effects on navrail and story buttons (Krille)
- design: Big redesign of three column mode to advanced two column mode (EricEisaman)
- design: Chat list design adjustments (Krille)
- design: Display last story as tiny message bubble in chat list (EricEisaman)
- design: Improve invite chat UX (EricEisaman)
- design: Move chatbackup in adaptive bottom sheet (EricEisaman)
- design: New three column layout for wide screens (EricEisaman)
- design: Nicer user bottom sheet (EricEisaman)
- design: Redesign style page (Krille)
- docs: Update readme (Krille)
- feat/ChatListItem: small changes (gilice)
- fix: Bootstrap on first try fails sometimes (EricEisaman)
- fix: Cancel search on back button tap on android (Krille)
- fix: Do not allow empty search server (EricEisaman)
- fix: First story appears to be unencrypted sometimes (EricEisaman)
- fix: Remove mpv and zenity to fix linux snap builds (EricEisaman)
- fix: Unable to send files from snap version (EricEisaman)
- refactor: Change group description to chat description (EricEisaman)
- refactor: Make router static (Krille)
- refactor: Migrate from pathsegment routing (Krille)
- refactor: Migrate routes to go router (EricEisaman)
...
- refactor: Update badge (EricEisaman)
- refactor: Update html build files (EricEisaman)
...
SigmaChat 1.7.0 features a new way to work with spaces via a bottom navigation bar. A lot of work
...
SigmaChat 1.6.0 features a lot of bug fixes and improvements. The code base has been
...
SigmaChat 1.3.0 makes it possible to report offensive users to server admins (not only messages).
It fixes
...
SigmaChat 1.3.0 is the first release with full null safe dart code. While this is a huge change
under the
...
SigmaChat 1.2.0 brings a new stories feature, a lot of bug fixes and improved
voice messages.
...
Minor bugfix release which fixes signing up on matrix.org and make SigmaChats voice messages
playable in Element.
...
- fix: Temporary disable sign-up for matrix.org (Currently gives "500: Internal Server Error" while
  SigmaChat **should** send the same requests like Element)
...
SigmaChat 0.32.0 targets improved stability and a new onboarding flow where single sign on is now
